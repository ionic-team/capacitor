...
const writeIndexHtmlOutput = (plugin, string) => {
  ...
  const targetDirName = pluginNameSplitCapitalized
    .slice(0, pluginNameSplitCapitalized.length - 1)
    .join('-')
    .toLowerCase();
  const p = path.join(
    SITE_DIR,
...
const generateIndexForPlugin = plugin => {
  let methodChildren = plugin.children.filter(
    m => m.name != 'addListener' && m.name != 'removeListener',
  );
  let listenerChildren = plugin.children.filter(
    m => m.name == 'addListener' || m.name == 'removeListener',
  );
  var html = [
  ...
  methodChildren.forEach(method => {
    ...
    method.signatures.forEach((signature, index) => {
      html.push(
        `<li><div class="avc-code-method-name"><anchor-link to="method-${method.name}-${index}">${method.name}()</anchor-link></div></li>`,
      );
    });
  });
  listenerChildren.forEach(method => {
    ...
    method.signatures.forEach((signature, index) => {
      ...
      html.push(
        `<li><div class="avc-code-method-name"><anchor-link to="method-${method.name}-${index}">${method.name}(${paramString})</anchor-link></div></li>`,
      );
    });
  });
  ...
  interfacesUsed.forEach(interface => {
    ...
    <div class="avc-code-interface" id="type-${interface.id}">
      ...
      <div class="avc-code-line">
        <span class="avc-code-keyword">${kindString.toLowerCase()}</span> <span class="avc-code-type-name">${
      interface.name
    }</span>
    ...
    if (interfaceDecl.children) {
      html.push(
        ...interfaceDecl.children.map(c => {
          const nameString = c.type.name
            ? c.type.name
            : c.type.value
            ? `'${c.type.value}'`
            : c.type.type && c.type.type === 'array'
            ? `${c.type.elementType.name}[]`
            : 'any';
          ...
          <div class="avc-code-interface-param">
            <div class="avc-code-param-comment">${
              (c.comment && `// ${c.comment.shortText}`) || ''
            }</div>
            <div class="avc-code-line"><span class="avc-code-param-name">${
              c.name
            }</span>
              ${
                c.flags && c.flags.isOptional
            ...
            (kindString !== 'Enum' &&
              ...
              ${
                (c.type.id &&
                  `<avc-code-type type-id="${c.type.id}">${nameString}</avc-code-type>`) ||
                `<avc-code-type>${nameString}</avc-code-type>`
              }`) ||
            ''
          };
            </div>
...
const getInterfacesUsedByMethod = method => {
  ...
  const interfaces = method.signatures.map(signature => {
    ...
    signature.type.typeArguments &&
      signature.type.typeArguments.forEach(arg => {
...
const generateMethodParamDocs = signature => {
  ...
  params &&
    ...
    */
      ...
      if (param.comment) {
        html.push(
          `<div class="avc-code-method-param-comment">${param.comment.text}</div>`,
        );
      }
      html.push('</div>');
    });
  ...
  <div class="avc-code-method-returns-info">
    <span class="avc-code-method-returns-label">Returns:</span> ${getReturnTypeName(
      signature.type,
    )}${
...
const generateMethodSignature = (method, signature, signatureIndex) => {
  ...
  parts.push(
    '<span class="avc-code-paren">)</span><span class="avc-code-return-type-colon">:</span> ',
  );
  ...
  parts.push(`
    ...
    ${
      (signature.comment &&
        `<div class="avc-code-method-comment">${signature.comment.shortText}</div>`) ||
      ''
    }
  `);
...
const getParamTypeName = param => {
  ...
  if (t == 'reference') {
    ...
    return `<avc-code-type>${generateReflectionType(
      param.type,
    )}</avc-code-type>`;
...
plugins.forEach(plugin =>
  writeDocumentationHtmlOutput(plugin, generateDocumentationForPlugin(plugin)),
);
plugins.forEach(plugin =>
  writeIndexHtmlOutput(plugin, generateIndexForPlugin(plugin)),
);
