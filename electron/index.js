...
const injectCapacitor = async function(url) {
  try {
    ...
    let pathing = path.join(url.substr(url.indexOf('://') + 3), '../../node_modules/@capacitor/electron/inject.js').replace(/\\/g,'\\\\');
    urlFileContents = urlFileContents.replace('<body>', `<body><script>window.require('${pathing}')</script>`);
