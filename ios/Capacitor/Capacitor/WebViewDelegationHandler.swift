...
open class WebViewDelegationHandler: NSObject, WKNavigationDelegate, WKUIDelegate, WKScriptMessageHandler, UIScrollViewDelegate, WKDownloadDelegate,
    ...
    open func webView(_ webView: WKWebView, decidePolicyFor navigationAction: WKNavigationAction, decisionHandler: @escaping (WKNavigationActionPolicy) -> Void) {
        ...
        // Handle Capacitor blob URLs (custom scheme; `blob:` is reserved by WebKit)
        if navURL.scheme == CAPBlobStore.scheme || navURL.absoluteString.hasPrefix(CAPBlobStore.urlPrefix) {
            decisionHandler(.allow)
            return
        }
