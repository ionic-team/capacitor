...
```json5
{
  ...
  "server": {
    ...
    "hostname": "app",
    // On iOS, it is possible to configure the local scheme that is used. This can be useful
    // when migrating from cordova-plugin-ionic-webview, where the default scheme is ionic.
    "scheme": "ionic",
