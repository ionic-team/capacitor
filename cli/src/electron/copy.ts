...
export async function copyElectron(config: Config) {
  ...
  const runtimePath = resolve(config.app.rootDir, 'node_modules', '@capacitor/voltage', 'dist/voltage.js');
  return await runTask(`Copying web assets from ${chalk.bold(webRelDir)} to ${chalk.bold(nativeRelDir)}`, async () => {
    ...
    await copy(webAbsDir, config.electron.webDirAbs);
    console.log(`Copying runtime...`);
    return copy(runtimePath, join(config.electron.webDirAbs, 'voltage.js'));
  });
}
