...
export async function installGradlePlugins(config: Config, capacitorPlugins: Plugin[], cordovaPlugins: Plugin[]) {
  ...
  const relativeCapacitorAndroidPath = convertToJavaPath(relative(settingsPath, capacitorAndroidPath));

  let settingsLines = `// DO NOT EDIT THIS FILE! IT IS GENERATED EACH TIME "capacitor update" IS RUN
include ':capacitor-android'
project(':capacitor-android').projectDir = new File('${relativeCapacitorAndroidPath}')
`;
  // add capacitor plugins
  capacitorPlugins.forEach(p => {
      const pluginPath = convertToJavaPath(relative(settingsPath, p.rootPath) + '/' + p.android!.path);
      settingsLines +=
`
include ':${getGradlePackageName(p.id)}'
project(':${getGradlePackageName(p.id)}').projectDir = new File('${pluginPath}')
`;
  ...
  cordovaPlugins.map( p => {
    const relativePluginPath = convertToJavaPath(relative(dependencyPath, p.rootPath));
...
export async function handleCordovaPluginsGradle(config: Config,  cordovaPlugins: Plugin[]) {
  ...
  cordovaPlugins.map( p => {
    const relativePluginPath = convertToJavaPath(relative(pluginsFolder, p.rootPath));
    const frameworks = getPlatformElement(p, platform, 'framework');
    frameworks.map((framework: any) => {
...
/**
 * Ensures that the argument is a valid java path.
 */
function convertToJavaPath(path: string): string {
