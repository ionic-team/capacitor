...
export async function installGradlePlugins(
  ...
  const enableCordova = cordovaPlugins.length > 0;
  const enableCordovaText = enableCordova
    ? `
...
`;
  ...
  const cordovaRepositoryText = enableCordova
    ? `
repositories {
    flatDir{
...
`
    : '';
  const cordovaApplyText = enableCordova
    ? `apply from: "../capacitor-cordova-android-plugins/cordova.variables.gradle"`
    : '';
  const cordovaDependencyText = enableCordova
...
${cordovaRepositoryText}
${cordovaApplyText}
dependencies {
${cordovaDependencyText}${capacitorPlugins
    .map((p) => {
