...
project(':capacitor-android').projectDir = new File('${relative(settingsPath, capacitorAndroidPath)}')
  ...
  cordovaPlugins.map( p => {
    ...
    frameworks.map((framework: any) => {
      if (framework.$.custom && framework.$.custom === 'true' && framework.$.type && framework.$.type === 'gradleReference') {
        applyArray.push(`apply from: "${relative(dependencyPath, p.rootPath)}/${framework.$.src}"`);
  ...
  cordovaPlugins.map( p => {
    ...
    frameworks.map((framework: any) => {
      if (!framework.$.type && !framework.$.custom) {
        ...
        applyArray.push(`apply from: "${relative(pluginsFolder, p.rootPath)}/${framework.$.src}"`);
      }
    });
    prefsArray = prefsArray.concat(getAllElements(p, platform, 'preference'));
  });
